...
async function processImages(arquivosCompletos) {
    ...
    console.log(await tensor.tensorImage.data());
...
async function processSingleImage(path) {
    ...
    const normalizedImage = channels === 3
        ? Array.from(data).map(v => v / 255).concat(Array.from({ length: 224 * 224 }, () => 1)) // Adiciona um canal adicional com todos os valores 1
        : Array.from(data).map(v => v / 255);

    // Criar um tensor 4D [1, 224, 224, 3] para uma única imagem
    const tensorImage = tf.tensor4d(normalizedImage, [1, 224, 224, 4]);
